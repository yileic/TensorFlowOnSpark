...
# one item in csv2 is `image | label`, to use input data in TFRecord format, change `--format csv` to `--format tfr`
...
## Running distributed MNIST training and inference using Spark ML Pipeline
```bash
# for CPU mode:
# export QUEUE=default
# remove references to $LIB_CUDA
